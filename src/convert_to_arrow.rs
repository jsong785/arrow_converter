...
pub fn create_reader<'a>(t: Type, r: impl std::io::Read + std::io::Seek + 'a) -> Result<Option<Box<dyn 'a + std::iter::Iterator<Item = Result<RecordBatch>>>>> {
    match t {
        Type::Csv => Ok(Some(Box::new(RecordBatchIteratorArrowAdapter{ inner: csv::create_reader(r, None)? } ))),
        Type::Json => Ok(Some(Box::new(RecordBatchIteratorArrowAdapter{ inner: json::create_reader(r, None)? } ))),
        Type::Xlsx => Ok(None),
        Type::Parquet => Ok(None),
    }
}

struct RecordBatchIteratorArrowAdapter<T: std::iter::Iterator<Item = arrow::error::Result<RecordBatch>>> {
    pub inner: T,
}

impl<T: std::iter::Iterator<Item = arrow::error::Result<RecordBatch>>> Iterator for RecordBatchIteratorArrowAdapter<T> {
...
pub(super) mod csv {
    ...
    pub(super) fn create_reader<R: Read + Seek>(reader: R, schema: Option<SchemaRef>) -> Result<Reader<R>> {
